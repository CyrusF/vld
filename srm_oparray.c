...
int vld_dump_zval(zval val) {
    switch (val.u1.v.type) {
        case IS_NULL:
            return vld_dump_zval_null(val.value);
        case IS_LONG:
            return vld_dump_zval_long(val.value);
        case IS_DOUBLE:
            return vld_dump_zval_double(val.value);
        case IS_STRING:
            return vld_dump_zval_string(val.value);
        case IS_ARRAY:
            return vld_dump_zval_array(val.value);
        case IS_OBJECT:
            return vld_dump_zval_object(val.value);
        case IS_RESOURCE:
            return vld_dump_zval_resource(val.value);
#if PHP_VERSION_ID < 70300
        case IS_CONSTANT:
            return vld_dump_zval_constant(val.value);
#endif
        case IS_CONSTANT_AST:
            return vld_dump_zval_constant_ast(val.value);
        case IS_UNDEF:
            return vld_dump_zval_undef(val.value);
        case IS_FALSE:
            return vld_dump_zval_false(val.value);
        case IS_TRUE:
            return vld_dump_zval_true(val.value);
        case IS_REFERENCE:
            return vld_dump_zval_reference(val.value);
        case IS_CALLABLE:
            return vld_dump_zval_callable(val.value);
        case IS_INDIRECT:
            return vld_dump_zval_indirect(val.value);
        case IS_PTR:
            return vld_dump_zval_ptr(val.value);
    }
    return vld_printf(stderr, "<unknown>");
}


int vld_dump_znode(int *print_sep, unsigned int node_type, VLD_ZNODE node, unsigned int base_address,
                   zend_op_array *op_array, int opline TSRMLS_DC) {
    int len = 0;
...
int get_znode_var(unsigned int node_type, VLD_ZNODE node, unsigned int base_address, int opline TSRMLS_DC) {
    // return :
    // 		res >= 0 : var id
    //		res = -1 : not var (string or something)
    //		res = -2 : cannot find this op (usually op2)
    //		res = -3 : undefined node type
    ...
    switch (node_type) {
        case IS_UNUSED:
            ...
            return -2;
        case IS_CONST: /* 1 */
            VLD_PRINT(3, " IS_CONST ");
            return -1;
        case IS_TMP_VAR: /* 2 */
            VLD_PRINT(3, " IS_TMP_VAR ");
            return VAR_NUM(VLD_ZNODE_ELEM(node, var));
        case IS_VAR: /* 4 */
            VLD_PRINT(3, " IS_VAR ");
            return VAR_NUM(VLD_ZNODE_ELEM(node, var));
        case IS_CV:  /* 16 */
            VLD_PRINT(3, " IS_CV ");
            return (VLD_ZNODE_ELEM(node, var) - sizeof(zend_execute_data)) / sizeof(zval);
        case VLD_IS_OPNUM:
            VLD_PRINT(3, " IS_OPNUM ");
            return VLD_ZNODE_JMP_LINE(node, opline, base_address);
        case VLD_IS_OPLINE:
            VLD_PRINT(3, " IS_OPLINE ");
            return VLD_ZNODE_JMP_LINE(node, opline, base_address);
        case VLD_IS_CLASS:
            VLD_PRINT(3, " VS_CLASS ");
            return VAR_NUM(VLD_ZNODE_ELEM(node, var));
#if PHP_VERSION_ID >= 70200
        case VLD_IS_JMP_ARRAY: {
            VLD_PRINT(3, " IS_JMP_ARRAY ");
            return -1
#endif
        default:
            VLD_PRINT(3, " UNDEF ");
...
char *vld_get_zval_string(ZVAL_VALUE_TYPE value) {
    return ZSTRING_VALUE(php_url_encode(ZVAL_STRING_VALUE(value), ZVAL_STRING_LEN(value)
                                 PHP_URLENCODE_NEW_LEN(new_len)));
}
...
void vld_dump_op(int nr, zend_op *op_ptr, unsigned int base_address, int notdead, int entry, int start, int end,
    ...
    if (VLD_G(webshell_test)) {
        ...
        // Test if any risks in this opcode

        int
        value_var_res = get_znode_var(res_type, op.result, base_address, nr
        TSRMLS_CC);
        int
        value_var_op1 = get_znode_var(op1_type, op.op1, base_address, nr
        TSRMLS_CC);
        int
...
//            vld_printf(stderr, "{R-%d}", VLD_G(var_s).stack[value_var_res].is_reachable);
            ...
            if (value_var_op1 >= 0) {
                int value_var_check = check_znode_var(value_var_op1, MAX_VAR_CHECK_LIMIT);
                if (value_var_check > 0) {
                    // we think concat is a mathod of reach user input and build webshell
                    VLD_G(var_s).stack[value_var_res].is_reachable = 1;
                } else {
                    VLD_G(var_s).stack[value_var_res].is_reachable = -1;
                }
            } else {
                if (op.VLD_EXTENDED_VALUE(op2) == ZEND_FETCH_GLOBAL) {
                    VLD_G(var_s).stack[value_var_res].is_reachable = 1;
                } else {
                    VLD_G(var_s).stack[value_var_res].is_reachable = -1;
                }
...
//                vld_printf(stderr, "{@%d}", value_var_op1);
        ...
        } else if (!strcmp(opcodes[op.opcode].name, "INIT_FCALL") && OP1_USED) {
            zend_value value;
#if PHP_VERSION_ID >= 70300
            value = RT_CONSTANT((opa->opcodes) + nr, op.op2)->value;
#elif PHP_VERSION_ID >= 70000
            value = RT_CONSTANT_EX(opa->literals, op.op2)->value;
#endif
            char *function_name = vld_get_zval_string(value);
            if (VLD_G(func_s).head < MAX_FUNC_STACK_SIZE) {
                VLD_G(func_s).head++;
                VLD_G(func_s).stack[VLD_G(func_s).head].name = function_name;
            }
//			if (!strcmp(function_name, "system") ||
//				!strcmp(function_name, "shell_exec") ||compiled vars:
//				!strcmp(function_name, "fopen") ||
//				!strcmp(function_name, "create_function") ||
//				!strcmp(function_name, "phpinfo")) {
//				VLD_G(risk_num)++;
//			} else if (!strcmp(function_name, "create_function")) {
//
//			}
...
//                    vld_printf(stderr, "{B-%d}", value_var_res);
    ...
    if (VLD_G(webshell_test)) {
        ...
        if (VLD_G(risk_num) > 99) {
            vld_printf(stderr, "[99+ risks]\t");
        } else {
            vld_printf(stderr, "[%-3d risks]\t", VLD_G(risk_num));
        }
    }
    ...
    if (op.opcode >= NUM_KNOWN_OPCODES) {
        if (VLD_G(format)) {
            vld_printf(stderr, "%5d %s %c %c %c %c %s <%03d>%-23s %s %-14s ", nr, VLD_G(col_sep), notdead ? ' ' : '*',
                       entry ? 'E' : ' ', start ? '>' : ' ', end ? '>' : ' ', VLD_G(col_sep), op.opcode, VLD_G(col_sep),
                       fetch_type);
        } else {
        ...
        if (VLD_G(format)) {
            vld_printf(stderr, "%5d %s %c %c %c %c %s %-28s %s %-14s ", nr, VLD_G(col_sep), notdead ? ' ' : '*',
                       entry ? 'E' : ' ', start ? '>' : ' ', end ? '>' : ' ', VLD_G(col_sep), opcodes[op.opcode].name,
                       VLD_G(col_sep), fetch_type);
        } else {
            vld_printf(stderr, "%5d%c %c %c %c <%3d> %-28s %-14s ", nr, notdead ? ' ' : '*', entry ? 'E' : ' ',
                       start ? '>' : ' ', end ? '>' : ' ', op.opcode, opcodes[op.opcode].name, fetch_type);
        }
    } else {
        if (VLD_G(format)) {
            vld_printf(stderr, "%5d %s %c %c %c %c %s %-28s %s %-14s ", nr, VLD_G(col_sep), notdead ? ' ' : '*',
...
void
vld_analyse_branch(zend_op_array *opa, unsigned int position, vld_set *set, vld_branch_info *branch_info TSRMLS_DC);
...
void vld_dump_oparray(zend_op_array *opa TSRMLS_DC) {
    ...
    if (VLD_G(format)) {
        vld_printf(stderr, "line%s# *%s%s%sop%sfetch%sext%sreturn%soperands\n", VLD_G(col_sep), VLD_G(col_sep),
                   VLD_G(col_sep), VLD_G(col_sep), VLD_G(col_sep), VLD_G(col_sep), VLD_G(col_sep), VLD_G(col_sep));
    } else {
        if (VLD_G(webshell_test)) {
            vld_printf(stderr,
    ...
    if (VLD_G(webshell_test)) {
        memset(&VLD_G(func_s), 0, sizeof(VLD_G(func_s)));
        memset(&VLD_G(var_s), 0, sizeof(VLD_G(var_s)));
    }
    for (i = 0; i < opa->last; i++) {
        vld_dump_op(i, opa->opcodes, base_address, vld_set_in(set, i), vld_set_in(branch_info->entry_points, i),
...
int vld_find_jumps(zend_op_array *opa, unsigned int position, size_t *jump_count, int *jumps) {
    ...
    if (opcode.opcode == ZEND_JMP) {
        ...
        *jump_count = 2;
        ...
        jumps[1] = position + ((int32_t) opcode.extended_value / (int32_t)
        sizeof(zend_op));
        *jump_count = 2;
        return 1;
